"""
Local LLM Analyzer Module
Analyzes CVs using locally-hosted LLM (Ollama/LM Studio/llama.cpp) for complete privacy.
Drop-in replacement for LLMAnalyzer (Groq) with same interface.

PRIVACY GUARANTEE:
- All data stays on your computer
- No external API calls
- No internet connection needed after model download
- CVs never leave your machine

SUPPORTED BACKENDS:
1. Ollama (RECOMMENDED) - easiest setup, best performance
2. LM Studio - GUI interface for model management
3. llama-cpp-python - advanced users, maximum control
...
# Import based on backend choice
BACKEND = os.environ.get('LOCAL_LLM_BACKEND', 'ollama')  # ollama, lmstudio, llamacpp
...
if BACKEND == 'ollama':
    try:
        ...
        raise ImportError(
            "Ollama Python package not found. Install with:\n"
            "pip install ollama\n\n"
    ...
    try:
        ...
        raise ImportError(
            "OpenAI package not found (needed for LM Studio compatibility). Install with:\n"
            "pip install openai"
        )
...
class LocalLLMAnalyzer:
    """
    Local LLM-based CV analyzer - 100% private, no external API calls.
    Compatible with Ollama, LM Studio, and llama-cpp-python.
    Drop-in replacement for LLMAnalyzer (Groq version).
    """
    ...
    def __init__(self,
        ...
        Args:
            ...
            backend: 'ollama', 'lmstudio', or 'llamacpp'
            model_path: Path to GGUF model file (only for llamacpp backend)
        """
        ...
        if model is None:
            if self.backend == 'ollama':
                ...
                model = model_path or os.environ.get('LOCAL_LLM_MODEL_PATH', '')
        ...
        self.cache_dir = Path(".cache/local_cv_analyses")
        if self.cache_enabled:
            self.cache_dir.mkdir(parents=True, exist_ok=True)
            logger.info(f"Cache enabled at: {self.cache_dir}")

        logger.info(f"✓ Local LLM initialized: {self.backend} - {self.model}")
        logger.info(f"✓ Privacy mode: All data stays on your computer")

    def _init_backend(self, model_path: Optional[str]):
        """Initialize the specific backend (Ollama/LM Studio/llama.cpp)."""
        if self.backend == 'ollama':
            ...
            logger.info(f"Using LM Studio at http://localhost:1234")
            logger.info("Make sure LM Studio server is running")

        elif self.backend == 'llamacpp':
            if not self.model or not Path(self.model).exists():
                raise ValueError(
                    f"Model path required for llamacpp backend.\n"
                    f"Download a GGUF model and set LOCAL_LLM_MODEL_PATH or pass model_path parameter.\n"
                    f"Recommended: https://huggingface.co/TheBloke/Llama-2-7B-Chat-GGUF"
                )
            self.client = Llama(
            ...
            logger.info(f"Using llama.cpp with model: {self.model}")

    def analyze_cv(self, cv_text: str) -> Dict[str, Any]:
        """
        Perform complete LLM-based analysis of a CV using local LLM.
        ...
        Returns:
            Dictionary with complete analysis results (same structure as LLMAnalyzer)
        """
        # Check cache first
        if self.cache_enabled:
        ...
        logger.info("Performing LOCAL LLM analysis (100% private)...")
    ...
    def analyze_skill_category(self,
        ...
        """
        Analyze a category of skills using local LLM.
    ...
    def _build_analysis_prompt(self,
        ...
        Same logic as LLMAnalyzer for consistency.
        """
        ...
        for skill_name, skill_info in skill_dict.items():
            keywords_str = ', '.join(skill_info['keywords'][:5])
    ...
    def _call_local_llm(self, prompt: str) -> Dict:
        """
        Call local LLM with retry logic.
        Supports Ollama, LM Studio, and llama.cpp backends.
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.error(f"LLM call error (attempt {attempt + 1}/{self.max_retries}): {e}")
                if attempt == self.max_retries - 1:
                    raise
                time.sleep(2 ** attempt)
    ...
    def _validate_and_adjust_score(self,
        ...
        # High scores (60+) require skill keyword in evidence or CV
        if adjusted_score >= 60:
            ...
            keyword_in_evidence = evidence_lower and any(kw.lower() in evidence_lower for kw in skill_keywords[:5])
            keyword_in_cv = any(kw.lower() in cv_lower for kw in skill_keywords[:3])

            if not keyword_in_evidence and not keyword_in_cv:
                # No evidence found - major penalty
                adjusted_score = max(adjusted_score - 40, 0)
                ...
                # Keyword in CV but not in evidence - moderate penalty
        ...
        # Apply time decay to scores based on recency_years
        if recency_years is not None and recency_years > 0:
            if recency_years <= 2:
                # Recent (0-2 years): no penalty
                recency_multiplier = 1.0
            elif recency_years <= 4:
                # Moderate age (2-4 years): 30% penalty
                recency_multiplier = 0.7
                validation_notes.append(f"Moderate age ({recency_years:.1f}y)")
            elif recency_years <= 5:
                # Old (4-5 years): 50% penalty
                recency_multiplier = 0.5
                validation_notes.append(f"Old experience ({recency_years:.1f}y)")
            else:
                # Very old (5+ years): 70% penalty
                recency_multiplier = 0.3
                validation_notes.append(f"Very old ({recency_years:.1f}y)")
        ...
        # Penalize generic/vague evidence phrases
        vague_phrases = ['familiar with', 'exposure to', 'basic knowledge',
                        'some experience', 'aware of', 'introduced to']
        evidence_lower = evidence.lower() if evidence else ""

        if adjusted_score >= 50 and evidence_lower and any(phrase in evidence_lower for phrase in vague_phrases):
            adjusted_score = max(adjusted_score - 25, 20)
        ...
        # Very high scores (80+) require concrete evidence
        if adjusted_score >= 80 and (not evidence or len(evidence) < 30):
            adjusted_score = max(adjusted_score - 15, 60)
            validation_notes.append("Insufficient evidence for high score")
            logger.debug(f"  ⚠ {skill_name}: Insufficient evidence for score 80+ - reduced to {adjusted_score}")
    ...
    def _parse_llm_response(self, llm_response: Dict, skill_dict: Dict, cv_text: str = "") -> Dict[str, Any]:
        ...
        Args:
            ...
            skill_dict: Skill configuration dictionary
            cv_text: Full CV text for validation

        Returns:
            Analysis dictionary with validated scores
        """
        ...
        for skill_name, skill_info in skill_dict.items():
            ...
            # Approximate matched keywords from evidence
    ...
    def _extract_keywords_from_evidence(self, evidence: str, keywords: List[str]) -> List[str]:
        """Extract matched keywords from evidence text."""
        if not evidence:
            return []
